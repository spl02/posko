...
export const UserSection = ({ initialAdmins }: { initialAdmins: AdminProfile[] }) => {
        ...
        {activeTab === "list" ? (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-600">
              <thead className="bg-slate-50/50 border-b border-slate-200/50">
                <tr>
                  <th className="px-8 py-5 font-bold text-xs uppercase tracking-widest text-slate-400">Pengguna</th>
                  <th className="px-8 py-5 font-bold text-xs uppercase tracking-widest text-slate-400">Role</th>
                  <th className="px-8 py-5 font-bold text-xs uppercase tracking-widest text-slate-400 text-right">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100/80">
                {admins.length === 0 ? (
                  ...
                  admins.map((admin) => (
                    <tr key={admin.id} className="transition-colors hover:bg-white/60 group">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-bold text-slate-900 text-base">{admin.full_name || "Tanpa Nama"}</p>
          ...
          <div className="flex flex-col items-center justify-center h-[400px] text-slate-400">
            <div className="p-4 bg-slate-100/50 rounded-3xl mb-4">
              <KeySquare size={48} className="text-slate-300" strokeWidth={1.5} />
            </div>
            <p className="font-bold text-slate-700 text-lg">Pengaturan Hak Akses</p>
            <p className="text-sm mt-1 max-w-sm text-center">Tentukan modul dasbor mana saja yang dapat diakses oleh peran tertentu (Sedang dalam pengembangan).</p>
          </div>
        )}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-white rounded-[2.5rem] p-8 shadow-2xl relative animate-in fade-in zoom-in duration-200">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-6">Undang Admin</h2>
            <div className="space-y-4">
              <div>
                <label className="text-sm font-bold text-slate-700 mb-1.5 block">Email Pengguna</label>
                <input type="email" placeholder="contoh@posko.com" className="w-full px-4 py-3 rounded-2xl bg-slate-50 border border-slate-200 outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" />
              </div>
              <button 
                onClick={() => {
                  alert('Fungsi invite email disiapkan di backend.');
