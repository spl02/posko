...
import { Plus, X, UploadCloud, Package, Image as ImageIcon } from "lucide-react";
...
export const ProductSection = ({ initialProducts }: { initialProducts: Product[] }) => {
  ...
  // Handle Pilih Gambar dengan Validasi 2MB
    ...
    <div className="space-y-6">
      ...
      {products.length === 0 ? (
        <div className="flex flex-col items-center justify-center bg-white/40 backdrop-blur-2xl rounded-[2.5rem] border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] h-[400px]">
          ...
          <p className="text-sm text-slate-500 mt-1">Tambahkan produk pertama Anda untuk mulai berjualan.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="relative w-full aspect-square bg-slate-100 overflow-hidden">
                ...
                <div className="absolute top-4 right-4 px-3 py-1.5 rounded-xl bg-white/90 backdrop-blur-md shadow-sm border border-white/50 text-xs font-bold text-slate-700">
                  Sisa: <span className={product.stock < 10 ? "text-red-500" : "text-emerald-500"}>{product.stock}</span>
                </div>
              </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="w-full max-w-lg bg-white rounded-[2.5rem] p-8 shadow-2xl relative animate-in fade-in zoom-in duration-200 my-8">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-6">Informasi Produk</h2>

            <div className="space-y-5">
              {/* Upload Area */}
              <div>
                <label className="text-sm font-bold text-slate-700 mb-2 block">Foto Produk (Max 2MB)</label>
                <div 
                ...
                >
                  {imagePreview ? (
                    ...
                    <>
                      ...
                      <span className="text-sm font-medium text-slate-500">Klik untuk unggah gambar</span>
                    </>
                  )}
                </div>
                <input 
              ...
              {/* Inputs */}
              <div>
                <label className="text-sm font-bold text-slate-700 mb-1.5 block">Nama Produk</label>
                <input 
                  type="text" 
                  value={name} 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-bold text-slate-700 mb-1.5 block">Harga (Rp)</label>
                  <input 
                    type="number" 
                    value={price} 
                ...
                <div>
                  <label className="text-sm font-bold text-slate-700 mb-1.5 block">Stok Tersedia</label>
                  <input 
                    type="number" 
                    value={stock} 
