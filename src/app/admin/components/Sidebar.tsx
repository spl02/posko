...
export const Sidebar = ({ isOpen, setIsOpen, menuItems, activeTabId, onTabClick }: SidebarProps) => {
  return (
    <aside 
      className={`
    ...
    >
      ...
      <nav className="mt-4 px-3 space-y-1 overflow-y-auto">
        {menuItems.map((item) => (
          ...
          >
            <item.icon size={20} className={activeTabId === item.id ? 'text-white' : 'text-slate-500'} />
            <span className="ml-3 text-nowrap">{item.title}</span>
            {activeTabId === item.id && <ChevronRight size={14} className="ml-auto" />}
          </button>
        ))}
